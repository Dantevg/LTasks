\section{Type representation}\label{section-task-types}
...
LuaStruct and struct.lua represent types at runtime by a default value. For example, from the LuaRocks description\footref{footnote-luastruct} of LuaStruct:
...
\subsubsection{Teal}
Teal\footnote{\url{https://github.com/teal-language/tl}} is a language that compiles to Lua. \dots \todo{expand}

\subsubsection{Luau}
Unlike Teal, Luau\footnote{\url{https://luau-lang.org/}} does not compile to Lua but has its own interpreter. \dots \todo{expand, types still present at runtime?}
