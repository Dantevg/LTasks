...
\end{minted}
\caption{Using the step combinator with \clean{OnValue} in Clean. It will automatically step once the user input is either a palindrome or a greeting.}
...
\end{minted}
\caption{Using the step combinator with \clean{OnAction} in Clean. It will only step when the user clicks ``Continue.''}
\label{lst:clean_step_onaction}
\end{subfigure}
\caption{\clean{OnValue} and \clean{OnAction} in iTasks. \clean{showInput} is a convenience wrapper around the iTasks function \clean{viewInformation}, \clean{isPalindrome} and \clean{isGreeting} are defined elsewhere.}
