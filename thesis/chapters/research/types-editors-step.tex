...
Do not specify types, but validate or convert task input. For example a task that expects a number can use the \lua{tonumber()} validator / conversion function and fail if the provided value is not accepted by the validator function.
